...
pub fn query_secure_status<E: XapQueryExecutor>(exec: &mut E) -> Result<XapSecureStatus> {
    Ok(exec.query(XapSecureStatusRequest(()))?.0.into())
}

pub fn query_config<E: XapQueryExecutor>(exec: &mut E) -> Result<(Config, String)> {
    //  data size
    let size = exec.query(QmkConfigBlobLengthRequest(()))?.0;

    //  all chunks and merge them in a Vec
    let mut data: Vec<u8> = Vec::with_capacity(size as usize);
    let mut offset: u16 = 0;
    while offset < size {
        let chunk = exec.query(QmkConfigBlobChunkRequest(offset))?;
...
pub fn query_key<E: XapQueryExecutor>(
    exec: &mut E,
    constants: &XapConstants,
    position: Point3D,
) -> Result<KeymapKey> {
    let code_raw = exec.query(KeymapGetKeycodeRequest(position.into()))?;
...
pub fn query_keymap<E: XapQueryExecutor>(
    exec: &mut E,
    constants: &XapConstants,
    info: &XapDeviceInfo,
    config: &Config,
...
pub fn query_encoder_keymap<E: XapQueryExecutor>(
    exec: &mut E,
    constants: &XapConstants,
    layer_count: u8,
    encoder_count: u8,
    ...
    for layer in 0..layer_count {
        ...
        for encoder in 0..encoder_count {
            ...
            for clockwise in 0..=1u8 {
                let raw = exec.query(KeymapGetEncoderKeycodeRequest(KeymapGetEncoderKeycodeArg {
                    layer,
                    encoder,
                    clockwise,
                }))?;
...
pub fn remap_key<E: XapQueryExecutor>(
    exec: &mut E,
    constants: &XapConstants,
    key: RemappingSetKeycodeArg,
) -> Result<KeymapKey> {
    exec.query(RemappingSetKeycodeRequest(key.clone()))?;
...
pub fn query_device_info<E: XapQueryExecutor>(
    exec: &mut E,
    constants: &XapConstants,
) -> Result<(XapDeviceInfo, Config, String)> {
    let subsystems = exec.query(XapEnabledSubsystemCapabilitiesRequest(()))?;

    let xap_info = XapInfo {
        version: exec.query(XapVersionRequest(()))?.0,
    };

    let qmk_caps = exec.query(QmkCapabilitiesRequest(()))?;
    let board_ids = exec.query(QmkBoardIdentifiersRequest(()))?;
    ...
    let (config, config_json) = query_config(exec)?;

    let hardware_id = exec.query(QmkHardwareIdentifierRequest(()))?.0;

    let qmk_info = QmkInfo {
        version: exec.query(QmkVersionRequest(()))?.0.to_string(),
        board_ids,
        manufacturer,
        product_name,
    ...
    let keymap_info = if subsystems.contains(XapEnabledSubsystemCapabilitiesFlags::Keymap) {
        let keymap_caps = exec.query(KeymapCapabilitiesRequest(()))?;

        let layer_count = if keymap_caps.contains(KeymapCapabilitiesFlags::GetLayerCount) {
            Some(exec.query(KeymapGetLayerCountRequest(()))?.0)
        } else {
            None
        };
    ...
    let remap_info = if subsystems.contains(XapEnabledSubsystemCapabilitiesFlags::Remapping) {
        let keymap_caps = exec.query(RemappingCapabilitiesRequest(()))?;

        let layer_count = if keymap_caps.contains(RemappingCapabilitiesFlags::GetLayerCount) {
            Some(exec.query(RemappingGetLayerCountRequest(()))?.0)
        } else {
            None
        };
    ...
    let lighting_info = if subsystems.contains(XapEnabledSubsystemCapabilitiesFlags::Lighting) {
        ...
        let rgblight_info = if lighting_caps.contains(LightingCapabilitiesFlags::Rgblight) {
            let rgblight_caps = exec.query(RgblightCapabilitiesRequest(()))?;

            let effects = if rgblight_caps.contains(RgblightCapabilitiesFlags::GetEnabledEffects) {
                exec.query(RgblightGetEnabledEffectsRequest(()))?.0
            } else {
                0
            };
        ...
        let rgbmatrix_info = if lighting_caps.contains(LightingCapabilitiesFlags::Rgbmatrix) {
            let rgbmatrix_caps = exec.query(RgbmatrixCapabilitiesRequest(()))?;

            let effects = if rgbmatrix_caps.contains(RgbmatrixCapabilitiesFlags::GetEnabledEffects) {
                exec.query(RgbmatrixGetEnabledEffectsRequest(()))?.0
            } else {
                0
            };
...
pub fn initialize<E: XapQueryExecutor>(
    exec: &mut E,
    constants: Arc<XapConstants>,
    id: Uuid,
) -> Result<XapDeviceState> {
    let (info, config, config_json) = query_device_info(exec, &constants)?;
    let keymap = query_keymap(exec, &constants, &info, &config)?;
    let secure_status = query_secure_status(exec)?;
...
mod test {
    ...
    impl XapQueryExecutor for FakeExecutor {
        fn query<T: XapRequest>(&mut self, _request: T) -> Result<T::Response> {
            use binrw::BinRead;
            let payload = self
                .responses
                .pop_front()
    ...
    fn query_config_decompresses_chunked_blob() {
        ...
        let (config, config_json) = query_config(&mut exec).expect("query_config");
    ...
    fn query_secure_status_decodes_via_executor() {
        ...
        let status = query_secure_status(&mut exec).expect("status");
