...
use xap_specs::{
    ...
    request::XapRequest,
    spec::keymap::{KeymapGetEncoderKeycodeArg, KeymapGetEncoderKeycodeRequest},
    spec::lighting::rgblight::{
        RgblightGetConfigRequest, RgblightSaveConfigRequest, RgblightSetConfigRequest,
    },
    spec::qmk::{QmkJumpToBootloaderRequest, QmkReinitializeEepromRequest},
    spec::remapping::{
        RemappingSetEncoderKeycodeArg, RemappingSetEncoderKeycodeRequest, RemappingSetKeycodeArg,
    },
    spec::types::RgbLightConfig,
    spec::xap::{XapSecureLockRequest, XapSecureUnlockRequest},
};

use xap_core::aggregation::keymap::MappedKeymap;
use xap_core::transport::XapQueryExecutor;
use xap_core::XapDeviceState;
use xap_core::XapEvent;

use super::hid::{device_matches, is_xap_device, spawn_worker, HidWriter};
...
pub struct XapClient {
    ...
    events: futures::channel::mpsc::UnboundedSender<XapEvent>,
}
...
impl XapClient {
    ...
    pub fn events_sink(&self) -> futures::channel::mpsc::UnboundedSender<XapEvent> {
        self.events.clone()
    }
    ...
    // One-shot command methods. These were Tauri-generated before; now
    // hand-written here. Each wraps a single `query` round-trip.
    pub fn secure_unlock(&mut self, id: Uuid) -> Result<()> {
        self.query(id, XapSecureUnlockRequest(())).map(|_| ())
    }
    pub fn secure_lock(&mut self, id: Uuid) -> Result<()> {
        self.query(id, XapSecureLockRequest(())).map(|_| ())
    }
    pub fn jump_to_bootloader(&mut self, id: Uuid) -> Result<u8> {
        Ok(self.query(id, QmkJumpToBootloaderRequest(()))?.0)
    }
    pub fn reinitialize_eeprom(&mut self, id: Uuid) -> Result<u8> {
        Ok(self.query(id, QmkReinitializeEepromRequest(()))?.0)
    }
    pub fn rgblight_get_config(&mut self, id: Uuid) -> Result<RgbLightConfig> {
        self.query(id, RgblightGetConfigRequest(()))
    }
    pub fn rgblight_set_config(&mut self, id: Uuid, cfg: RgbLightConfig) -> Result<()> {
        self.query(id, RgblightSetConfigRequest(cfg)).map(|_| ())
    }
    pub fn rgblight_save_config(&mut self, id: Uuid) -> Result<()> {
        self.query(id, RgblightSaveConfigRequest(())).map(|_| ())
    }
    pub fn encoder_keycode_get(&mut self, id: Uuid, arg: KeymapGetEncoderKeycodeArg) -> Result<u16> {
        Ok(self.query(id, KeymapGetEncoderKeycodeRequest(arg))?.0)
    }
    pub fn encoder_keycode_set(
        &mut self,
        id: Uuid,
        arg: RemappingSetEncoderKeycodeArg,
    ) -> Result<()> {
        self.query(id, RemappingSetEncoderKeycodeRequest(arg))
            .map(|_| ())
    }
