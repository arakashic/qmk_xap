...
describe('templateFill', () => {
  it('startFill on LT opens a tap hole', () => {
    const p = startFill({ layer: 0, row: 1, column: 2 }, { kind: 'LT' }, { layer: 2 })!
  ...
  it('startFill on non-parameterized kind returns null', () => {
    expect(startFill({ layer: 0, row: 0, column: 0 }, { kind: 'MO' }, {})).toBeNull()
    expect(startFill({ layer: 0, row: 0, column: 0 }, { kind: 'TG' }, {})).toBeNull()
  })

  it('startFill on MT opens a tap hole', () => {
    const p = startFill({ layer: 0, row: 0, column: 3 }, { kind: 'MT', mods: ['MOD_LSFT'] }, { mod_mask: 2 })!
