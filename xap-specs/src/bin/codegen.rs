...
type Wrap = <T>(p: Promise<T>) => Promise<{ status: 'ok'; data: T } | { status: 'error'; error: string }>

export const generatedCommands = (
    ensureClient: () => Promise<XapWasmClient>,
    wrap: Wrap,
) => ({";
...
impl Route {
    ...
    fn render_command(&self, ctx: &mut Context) -> Result<String> {
        ...
        // --- Layer 1 generated browser surface: wasm method + TS command entry ---
        let name_camel = name.to_case(Case::Camel);
        let module_path = ctx.full_module_path();
        match self.request_type {
            BasicType::Unit => {
                writeln!(
                    &mut ctx.wasm_rpc_file,
                    r#"
                    pub fn {name_snake}(&self, device_id: String) -> js_sys::Promise {{
                        let inner = self.inner.clone();
                        wasm_bindgen_futures::future_to_promise(async move {{
                            let id = Uuid::parse_str(&device_id).map_err(jserr_str)?;
                            let resp = query(inner, id, xap_specs::spec::{module_path}::{name_pascal}Request(())).await?;
                            to_js(&resp)
                        }})
                    }}"#
                )?;
                writeln!(
                    &mut ctx.browser_commands_file,
                    "    {name_camel}: (id: string) => wrap(ensureClient().then(c => c.{name_snake}(id))),"
                )?;
            }
            _ => {
                let arg_type = match &self.request_type {
                    BasicType::Struct => {
                        format!("xap_specs::spec::{module_path}::{request_type_name}")
                    }
                    BasicType::Predefined(_) => {
                        format!("xap_specs::spec::types::{request_type_name}")
                    }
                    _ => request_type_name.clone(),
                };
                writeln!(
                    &mut ctx.wasm_rpc_file,
                    r#"
                    pub fn {name_snake}(&self, device_id: String, arg: JsValue) -> js_sys::Promise {{
                        let inner = self.inner.clone();
                        wasm_bindgen_futures::future_to_promise(async move {{
                            let id = Uuid::parse_str(&device_id).map_err(jserr_str)?;
                            let arg: {arg_type} = serde_wasm_bindgen::from_value(arg).map_err(jserr_str)?;
                            let resp = query(inner, id, xap_specs::spec::{module_path}::{name_pascal}Request(arg)).await?;
                            to_js(&resp)
                        }})
                    }}"#
                )?;
                writeln!(
                    &mut ctx.browser_commands_file,
                    "    {name_camel}: (id: string, arg: unknown) => wrap(ensureClient().then(c => c.{name_snake}(id, arg))),"
                )?;
            }
        }
...
struct Context<'a> {
    ...
    wasm_rpc_file: &'a mut dyn Write,
    browser_commands_file: &'a mut dyn Write,
    commands: Vec<String>,
    spec: &'a Spec,
...
fn get_default_wasm_rpc_file() -> &'static str {
    concat!(env!("CARGO_MANIFEST_DIR"), "/../xap-wasm/src/generated.rs")
}

fn get_default_browser_commands_file() -> &'static str {
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../src/xap-runtime/generated-commands.ts"
    )
}
...
struct Args {
    ...
    #[arg(long, default_value = get_default_wasm_rpc_file())]
    wasm_rpc_file: PathBuf,
    #[arg(long, default_value = get_default_browser_commands_file())]
    browser_commands_file: PathBuf,
...
fn main() -> Result<()> {
    ...
    if let Some(spec) = specs.last() {
        ...
        let mut tauri_rpc_file = File::create(&args.tauri_rpc_file)?;
        let mut wasm_rpc_file = File::create(&args.wasm_rpc_file)?;
        let mut browser_commands_file = File::create(&args.browser_commands_file)?;

        writeln!(wasm_rpc_file, "{WASM_HEADER}")?;
        writeln!(browser_commands_file, "{TS_HEADER}")?;

        {
            let mut context = Context {
                xap_spec_file: &mut xap_spec_file,
                tauri_rpc_file: &mut tauri_rpc_file,
                wasm_rpc_file: &mut wasm_rpc_file,
                browser_commands_file: &mut browser_commands_file,
                commands: Vec::new(),
                module_path: Vec::new(),
                spec,
            };
        ...
        // Close the generated impl block and the factory object.
        writeln!(wasm_rpc_file, "}}")?;
        writeln!(browser_commands_file, "}})")?;
    }
    ...
    if args.format {
        ...
        format_file(&args.wasm_rpc_file)?;

        // Format the generated TypeScript with the project's prettier.
        let status = Command::new("npx")
            .arg("prettier")
            .arg("--write")
            .arg(&args.browser_commands_file)
            .status()?;
        if !status.success() {
            bail!(
                "prettier failed on {:?} ({status})",
                args.browser_commands_file
            );
        }
    }
