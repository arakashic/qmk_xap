...
struct Spec {
    ...
    // BTreeMap (not HashMap) so the emitted `types` structs render in a stable,
    // key-sorted order — keeps codegen output deterministic/reproducible.
    #[serde(default)]
...
fn main() -> Result<()> {
    ...
    if let Some(spec) = specs.last() {
        ...
        let mut xap_spec_file = File::create(&args.xap_spec_file)?;

        {
            let mut context = Context {
                xap_spec_file: &mut xap_spec_file,
                module_path: Vec::new(),
                spec,
            };
    ...
    if args.format {
        // Formatter failures are fatal: the committed generated artifacts are
        // expected to be formatted, so a silent formatter failure (e.g. rustfmt
        // rejecting the wrong edition) must not pass as success.
        fn format_file(rendered_file: impl AsRef<Path>) -> Result<()> {
            ...
            let status = Command::new("rustfmt")
                ...
                .status()?;
            if !status.success() {
                bail!("rustfmt failed on {rendered_file:?} ({status})");
            }
            Ok(())
        }
